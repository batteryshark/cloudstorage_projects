'''
	Google Drive Recovery Utility by rFx
	Description: This tool parses all files
	in your Google Drive and finds files with,
	for whatever reason (no judgement), no parents.
	It then puts them in a directory in root called
	"Recovered"
'''
	...
	while True:
		try:
			...
			for fd in files['items']:
				if(fd['parents'] == []):
					print("Found %s" % get_filename(fd['id']))
